...
async def my_files(bot: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text("Error fetching your files. Please try again later.")
        return
...
async def tos_handler(bot: Client, message: Message):
    ...
    if not await is_user_accepted_tos(message):
        await message.reply_text(
            Var.TOS,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ I accept the TOS", callback_data="accepttos_1")]])
        )
    else:
        await message.reply_text("You have already accepted the Terms of Service.")
...
async def info_handler(bot: Client, message: Message):
    ...
    try:
        ...
        await message.reply_text("Error fetching user info. Please try again later.")
...
@StreamBot.on_message(filters.command('accepttos') & filters.private)
async def accept_tos_handler(bot: Client, message: Message):
    try:
        user_id = message.from_user.id
        await db.agreed_tos(user_id)
        await message.reply_text("✅ You have accepted the Terms of Service.")
    except Exception as e:
        await message.reply_text("Error processing your request. Please try again later.")
